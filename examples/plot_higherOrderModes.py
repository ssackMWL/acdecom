...
"""
Empty circular pipe with flow and higher-order modes
===========================================
...
# The duct radius is 0.075 m. The distance from the microphones to the reference cross-section are 0.55 m.
...
f_max = 3200  # Hz
# %%
# The measurement must be pre-processed in a format that is understood by the *WaveGuide* object. In general,
# this must be a numpy.ndarray, where the columns contain the measurement data, such as the measured frequency, the
...
# %%
# We can have a look at the file header to understand how the data is stored in our input file.
with open("data/higherOrderModes.txt") as pressurefile:
    print(pressurefile.readline().split(","))
# %%
# Mach-number, temperature, and frequency are stored in column 0, 1, and 2. The upstream microphones 1-12 are in
# column 3 - 14, the downstream microphones 13-24 are in column 15-26, and the case number is in the last column.
Mach_number = 0
temperature = 1
f = 2
mics_us = range(3, 15)
...
#
# .. note::
#   In our case, the microphone coordinates are defined in a cylindrical coordinate system with the circumferential
#   position in deg. We therefore set the argument *cylindrical_coordinates* to *True*. This will transform the
...
print(headers_ds)
# %%
# We use that information extract the modal data for the different frequencies and cases.
