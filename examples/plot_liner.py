...
"""
Noise Scattering at a Compressor Upstream and Downstream
==================================================
...
# The highest frequency of interest is 1000 Hz. The bulk Mach-number was 0.25 and the temperature was 295 Kelvin.
section = "rectangular"
a = 0.02  # m
b = 0.11  # m
f_max = 1500  # Hz
...
# 3 microphones, each. The first microphones had a distance of 0.21 m to the liner.
distance_upstream = 0.21  # m
distance_downstream = 0.21  # m
# %%
# To analyse the measurement data, we create objects for the upstream and the downstream test ducts.
...
#
# .. note::
#   The standard flow direction is in :math:`P_+` direction. On the intake side, the Mach-number therefore must be
...
#   This is more accurate than the often used model by Kirchoff. However, it is computationally more expensive.
#
# We define lists with microphone positions at the upstream and downstream side and assign them to the *WaveGuide*s.
...
# *WaveGuide* object. In general, this must be a numpy.ndarray, where the columns contain the measurement data, such
# as the measured frequency and the pressures at the microphone locations. The rows can be different frequencies or
#  different sound excitations (cases). In this example, the measurement was post-processed into a *.txt file and can
#  be loaded with the `numpy.loadtxt <https://numpy.org/doc/stable/reference/generated/numpy.loadtxt.html>`_ function.
#
# .. note::
#   The pressure used for the decomposition must be pre-processed, e.g., to account for microphone calibration if
#   necessary.
#
pressure = numpy.loadtxt("data/liner.txt", dtype=complex, delimiter=",", skiprows=1)
# %%
# We can have a look at the file's header to understand how the data is stored in our input file.
with open("data/liner.txt") as pressurefile:
    print(pressurefile.readline().split(","))
# %%
# The intake microphones (1, 2, and 3) are in column 5, 6, and 7. The Downstream microphones
# (3, 5, and 6) are in column 8, 9, and 10. The case number is in the last column. All the other columns contain
...
#   modes. The condition number of the wave-decomposition is stored in the data returned by *modalDecomposition* and
...
# %%
# We use that information extract the modal data.
minusmodes = [1]  # from headers_us
plusmodes = [0]
## %%
# We can furthermore get the unique decomposed frequency points.
...
# acoustic modes as rows and the different test cases as columns.
#
# .. note::
...
# %%
# We can plot the transmission and reflection coefficients at the intake and Downstream.
...
plt.plot(frequs, TLUpstream, ls="-", color="#67A3C1", label="Transmission Loss (US)")
plt.plot(frequs, dissipation_us, ls="--", color="#67A3C1", label="Dissipation (US)")
plt.plot(frequs, TLDownstream, ls="-", color="#D38D7B", label="Transmission Loss (DS)")
plt.plot(frequs, dissipation_ds, ls="--", color="#D38D7B", label="Dissipation (DS)")
plt.xlabel("Frequency [Hz]")
