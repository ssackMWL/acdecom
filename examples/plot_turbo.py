...
"""
Noise Scattering at a Compressor Inlet and Outlet
==================================================

 In this example, we extract the scattering of noise at a compressor inlet and outlet. Besides the pressure measured
 with flush-mounted microphones, we will use the temperature, and flow velocity that was acquired during the
 measurement. The data is part of a study performed at the
...
import acdecom
# %%
# Compressor intake and outlet have a circular cross-section of the radius 0.026 m and 0.028 m.
...
f_max = 3200  # Hz
# %%
# During the test, test-ducts were mounted to the intake and outlet. Those ducts were equipped with 3 microphones, each.
# The first microphone had a distance to the Intake of 0.73 m and 1.17 m to the outlet.
distance_intake = 0.073  # m
distance_outlet = 1.17  # m
# %%
# To analyse the measurement data, we create objects for the intake and the outlet test pipes.
...
#
# We define lists with microphone positions at the intake and outlet and assign them to the *WaveGuide*s.
...
# The Mach-numbers at the intake and outlet are stored in column 0 and 1, the temperatures in column 2 and 3,
# and the frequency in column 4. The intake microphones (1, 2, and 3) are in column 5, 6, and 7. The outlet microphones
# (3, 5, and 6) are in column 8, 9, and 10. The case number is in the last column.
...
# Now, we can decompose the sound-fields into the propagating modes. We decompose the sound-fields on the intake
